...
def Cash_Check (dbf, zabbix_filename):
    ...
    for keys, values in add_list.items():
        Add_Into_Zabbix(zabbix, keys, router_id, values)
    zabbix.close()
...
def Add_Into_Zabbix( file, keys, router_id, values): 
    s1 =  '{0:16}  {1:15}  {2:7}'.format(keys, router_id, values)
    s = s1 + '   ' + datetime.strftime(datetime.now(), "%d.%m.%Y_%H:%M:%S")
    #s = keys + '   ' + router_id + '   ' + values + '   ' + datetime.strftime(datetime.now(), "%d.%m.%Y_%H:%M:%S")
    file.write (s + '\n')
